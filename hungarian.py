...
model = pickle.load(open("model/XGB model.pkl", 'rb'))
...
with tab1:
    ...
    thalach = st.sidebar.number_input(label=":violet[**Maximum heart rate achieved**]",
                                      min_value=df_final['thalach'].min(), max_value=df_final['thalach'].max())
    st.sidebar.write(
    ...
    st.write("")
    if predict_btn:
        inputs = [[age, sex, cp, trestbps, chol,
...
with tab2:
    ...
    st.download_button("Download CSV Example", data=sample_csv,
                       file_name='sample_heart_disease_parameters.csv', mime='text/csv')
